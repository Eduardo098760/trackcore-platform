  ...
  if (!isClient || loading) {
    return (
      <div className="h-screen w-screen flex flex-col items-center justify-center bg-background">
        ...
        <p className="text-muted-foreground text-xs mt-1">TrackCore · Compartilhamento Seguro</p>
      </div>
    );
  }
  ...
  if (expired || (data && !data.valid)) {
    return (
      <div className="h-screen w-screen flex flex-col items-center justify-center bg-background px-6">
        ...
        <h1 className="text-xl font-bold text-foreground mb-2">Link Expirado</h1>
  ...
  return (
    <div className="h-screen w-screen relative bg-background overflow-hidden">
      ...
      {isClient && !pos && (
        <div className="absolute inset-0 z-0 flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-muted-foreground text-sm">Aguardando posição GPS…</p>
          </div>
        </div>
      )}
      ...
      <div className="absolute top-3 left-3 right-3 z-[1000] flex items-center justify-between gap-3">
        {/* Brand */}
        <div className="flex items-center gap-2 bg-popover/90 backdrop-blur-md border border-border rounded-xl px-3 py-2 shadow-lg">
          ...
          <span className="text-foreground text-xs font-bold tracking-wide">TrackCore</span>
          <div className="w-px h-3 bg-white/20" />
          <span className="text-muted-foreground text-[11px]">Modo leitura</span>
        </div>

        {/* Countdown */}
        {data?.expiresAt && (
          <div className="bg-popover/90 backdrop-blur-md border border-border rounded-xl px-3 py-2 shadow-lg">
      ...
      <div className="absolute bottom-4 left-3 right-3 z-[1000]">
        <div className="bg-background/92 backdrop-blur-xl border border-border rounded-2xl shadow-2xl overflow-hidden">
          ...
          <div className="flex items-center justify-between px-4 pt-4 pb-3 border-b border-border/50">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-foreground font-semibold text-sm leading-tight">{dev?.name || 'Veículo'}</p>
                {dev?.plate && (
                  <p className="text-[11px] font-mono text-muted-foreground mt-0.5">{dev.plate}</p>
                )}
              </div>
            </div>
            <span
          ...
          {pos && (
            <div className="grid grid-cols-3 gap-2 px-4 py-3">
              <div className="bg-muted/15 rounded-xl p-2.5 border border-border/50">
                <div className="flex items-center gap-1 mb-1.5">
                  <Gauge className="w-3 h-3 text-blue-400" />
                  <span className="text-[9px] uppercase font-semibold text-muted-foreground tracking-wide">Velocidade</span>
                </div>
                <p className="text-foreground font-bold text-base leading-none">
                  {Math.round(pos.speed)}<span className="text-muted-foreground text-[10px] font-normal ml-0.5">km/h</span>
                </p>
              </div>
              <div className="bg-muted/15 rounded-xl p-2.5 border border-border/50">
                <div className="flex items-center gap-1 mb-1.5">
                  <Zap className="w-3 h-3 text-yellow-400" />
                  <span className="text-[9px] uppercase font-semibold text-muted-foreground tracking-wide">Ignição</span>
                </div>
                <p className={`font-bold text-base leading-none ${pos.attributes.ignition ? 'text-green-400' : 'text-muted-foreground'}`}>
              ...
              <div className="bg-muted/15 rounded-xl p-2.5 border border-border/50">
                <div className="flex items-center gap-1 mb-1.5">
                  ...
                  <span className="text-[9px] uppercase font-semibold text-muted-foreground tracking-wide">Satélites</span>
                </div>
                <p className="text-foreground font-bold text-base leading-none">
                  {pos.attributes.sat ?? '—'}
                </p>
          ...
          {pos && (
            <div className="px-4 pb-3">
              <div className="flex items-start gap-2 bg-muted/10 border border-border/50 rounded-xl p-2.5">
