 ...
 * Usa o número configurado no tenant. Não renderiza se não houver número.
 * Inclui automaticamente os dados do usuário logado na mensagem.
 */
export function WhatsAppButton() {
  ...
  const parts = [
    tenant.whatsappMessage || 'Olá! Preciso de suporte.',
    '',
    '--- Dados do usuário ---',
  ];
  if (user?.name) parts.push(`Nome: ${user.name}`);
  if (user?.email) parts.push(`Email: ${user.email}`);
  if (user?.phone) parts.push(`Telefone: ${user.phone}`);
