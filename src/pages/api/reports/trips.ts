...
const TRACCAR_URL =
  process.env.TRACCAR_API_URL || process.env.NEXT_PUBLIC_API_URL || "http://localhost:8082";
    ...
    const reports = await Promise.all(
      deviceIds.map(async (deviceId) => {
        try {
          ...
          console.log(
            `[Trips API] Trips retornados para device ${deviceId} (normalizadas):`,
            trips.length,
          );
          ...
          const formattedTrips = await Promise.all(
            trips.map(async (trip: any, index: number) => {
              const [startAddress, endAddress] = await Promise.all([
                geocodeAddr(
                  trip.startLat ?? trip.startLatitude,
                  trip.startLon ?? trip.startLongitude,
                  trip.startAddress,
                ),
                geocodeAddr(
